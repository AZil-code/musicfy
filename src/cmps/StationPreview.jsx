...
export function StationPreview({ station, isColapsed, isSelected, modalVersion=false, songToAdd=null, onPlay}) {
   ...
   const subtitle = station.sourceCreator || station.createdBy?.fullname || station.createdBy?.username || 'Musicfy'
   const stationTags = Array.isArray(station.tags) ? station.tags.map((tag) => String(tag).toLowerCase()) : []
   const typeLabel = stationTags.includes('album') ? 'Album' : 'Playlist'
   ...
   return (
      <div className={`station-preview ${isColapsed ? 'station-preview-collapsed' : '' }`}>
